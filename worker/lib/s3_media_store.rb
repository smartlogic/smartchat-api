class S3MediaStore
  def initialize(private_bucket, published_bucket)
    @private_bucket, @published_bucket = private_bucket, published_bucket
  end
  ...
  def publish(path, user_id, media_id, encryptor)
    ...
    object = @published_bucket.objects[public_file_path]
